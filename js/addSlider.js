  ...
  Object.defineProperty(this, "value", {
    ...
    set: function (newVal) {
      ...
      if (!this._settings.range) {
        var offset =
          betterParseFloat(this._settings.value) - this._settings.min;
        var per =
          (toNearest(offset, this._settings.step) /
        ...
        this._elements
          ...
          .html(
            toFunc(this._settings.formatter).call(this, this._settings.value)
          );
      } else {
        var l = toNearest(
        ...
        this._elements.each(function (i, el) {
          ...
          $el
            ...
            .eq(0)
            .find(".addui-slider-value span")
            .html(toFunc(self._settings.formatter).call(self, l));
          $el
            .find(".addui-slider-handle")
            .eq(1)
            .find(".addui-slider-value span")
            .html(toFunc(self._settings.formatter).call(self, h));
        });
      }
    },
  });
  ...
  this.renderer = function () {
    ...
    if (!this._settings.range) {
      ...
      function dragHandler(e) {
        ...
        var val =
          toNearest(
            (offsetPer / 100) * (self._settings.max - self._settings.min),
            self._settings.step
          ) + self._settings.min;
      ...
      function dragHandler1(e) {
        ...
        var val = toNearest(
          offsetVal + self._settings.min,
          self._settings.step
        );
        val = Math.min(self._settings.max, Math.max(self._settings.min, val));
        var higherVal = toNearest(
          betterParseFloat(self._settings.value.split(",")[1]),
          self._settings.step
        );
      ...
      function dragHandler2(e) {
        ...
        val = Math.min(self._settings.max, Math.max(self._settings.min, val));
        var lowerVal = toNearest(
          betterParseFloat(self._settings.value.split(",")[0]),
          self._settings.step
        );
  ...
  this.init = function (settings) {
    ...
    if (!this._settings.range) {
      this._settings.value = Math.max(
        this._settings.min,
        Math.min(this._settings.max, betterParseFloat(this._settings.value))
      );
    } else {
      ...
      if (v1 < v2) this._settings.value = v1 + "," + v2;
      else this._settings.value = v2 + "," + v1;
    }
...
$add.auto.Slider = function () {
  if (!$add.auto.disabled) $("[data-addui=slider]").addSlider();
};
$(function () {
  $add.auto.Slider();
});
